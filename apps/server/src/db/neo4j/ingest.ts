...
async function runWithRetry(
  ...
  query: string,
  params: Record<string, PropertyValue | PropertyRecord | Array<Record<string, PropertyValue | PropertyRecord>> | null>,
  maxRetries = 5,
) {
  let attempt = 0;
  ...
  while (true) {
    try {
      ...
      if (attempt >= maxRetries || !(retryable || isDeadlock)) throw error;
...
function isPrimitive(v: Primitive | Primitive[] | undefined): v is Primitive {
  return v === null || ["string", "number", "boolean"].includes(typeof v);
}

function sanitizeValue(v: Primitive | Primitive[] | undefined): PropertyValue {
  if (v === undefined) return null;
  if (isPrimitive(v)) return v;
  return v.filter(isPrimitive);
}

function sanitizeProps(props: IRNode["props"] | IREdge["props"] | undefined): PropertyRecord {
...
function labelForKind(kind: IRNodeKind): IRNodeKind {
  return kind;
}
