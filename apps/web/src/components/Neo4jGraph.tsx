...
export function Neo4jGraph({
  ...
  graphAction?: 'load' | 'clear' | undefined;
}) {
  // --- Filter/Search State ---
  ...
  // Set graphLoaded to true after first non-empty graph
  ...
  }, [edges, filteredNodeIds, nodes, collapseCallSites]);
    ...
    const simulation = d3
      ...
      .force(
        ...
        d3
          ...
          .id((d) => String(d.id))
          .distance((l) => {
            // Increase distance for certain types
...
function truncateLabel(label: string, max = 28): string {
  ...
  return label;
    ...
    node.on("click", function (_event: MouseEvent, d) {
      ...
      if (d.labels?.includes('CallSiteSummary')) {
        const parentId = d.properties?.parentId;
    ...
    node.selectAll("circle")
      .attr("stroke-width", (d: any) =>
        selectedNodeId && String((d as SimNode).id) === selectedNodeId ? 4 :
        hoveredNodeId && String((d as SimNode).id) === hoveredNodeId ? 3 : 1.5)
      .attr("stroke", (d: any) =>
        selectedNodeId && String((d as SimNode).id) === selectedNodeId ? "#fbbf24" :
        hoveredNodeId && String((d as SimNode).id) === hoveredNodeId ? "#38bdf8" : "#f9fafb")
      .attr("fill-opacity", (d: any) =>
        selectedNodeId && !neighborIds.has(String((d as SimNode).id)) ? 0.1 :
        hoveredNodeId && String((d as SimNode).id) === hoveredNodeId ? 1 : 1
      )
      .transition().duration(180)
      .attr("r", (d: any) =>
        hoveredNodeId && String((d as SimNode).id) === hoveredNodeId ? nodeRadius(d) + 4 : nodeRadius(d)
      );
    node.selectAll("rect")
      .attr("opacity", (d: any) =>
        selectedNodeId && !neighborIds.has(String((d as SimNode).id)) ? 0.1 :
        hoveredNodeId && String((d as SimNode).id) === hoveredNodeId ? 1 : 0.92
      )
      .transition().duration(180)
      .attr("width", (d: any) => {
        const label = truncateLabel(nodeLabel(d));
        return hoveredNodeId && String((d as SimNode).id) === hoveredNodeId ? (10 * label.length + 24) : (10 * label.length + 14);
      });
    node.selectAll("text")
      .attr("fill", (d: any) =>
        selectedNodeId && !neighborIds.has(String((d as SimNode).id)) ? "#888" :
        hoveredNodeId && String((d as SimNode).id) === hoveredNodeId ? "#38bdf8" : "#f3f4f6"
      )
      .transition().duration(180)
      .style("font-size", (d: any) => hoveredNodeId && String((d as SimNode).id) === hoveredNodeId ? "18px" : "16px");
    ...
    link.attr("stroke-opacity", (l: any) => {
      ...
      if (selectedNodeId && !(getId(l.source) === selectedNodeId || getId(l.target) === selectedNodeId)) return 0.07;
      if (hoveredNodeId && !(getId(l.source) === hoveredNodeId || getId(l.target) === hoveredNodeId)) return 0.15;
      return 0.55;
    });
    link.attr("stroke", (l: any) => {
      ...
      if (hoveredNodeId && (getId(l.source) === hoveredNodeId || getId(l.target) === hoveredNodeId)) return "#38bdf8";
      return "#4b5563";
    });

    // Run simulation for a few more ticks for better layout, then stop
    const TICKS = selectedNodeId ? 0 : 120;
    let tickCount = 0;
    simulation.on("tick", () => {
      link
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{gap: 0, position: 'relative'}}>
      {/* --- Filter/Search UI --- */}
      <div style={{
      ...
      {/* --- Selected node types display removed for compactness --- */}
      {/* --- Graph --- */}
      <div ref={containerRef} className="w-full h-full" style={{flex: 1, minHeight: 0, background: 'radial-gradient(ellipse at center, #18181b 60%, #111112 100%)', position: 'relative'}}>
            ...
            <div>
              ...
              <ul style={{margin: '8px 0 0 18px', fontSize: 14}}>
                {(() => {
                  ...
                  const rels = simLinks.filter(l => getId(l.source) === selectedNode.id || getId(l.target) === selectedNode.id);
...
function nodeRadius(n: Neo4jNode): number {
  ...
  if (hasLabel(n, "Repo")) return 30;
  if (hasLabel(n, "Directory")) return 20;
  if (hasLabel(n, "CodeFile") || hasLabel(n, "TextFile")) return 14;
...
function nodeLabel(n: Neo4jNode): string {
  ...
  return (
    lastSegment(p.name) ||
    lastSegment(p.relPath) ||
    lastSegment(p.fileRelPath) ||
    lastSegment(p.normalized) ||
    lastSegment(p.raw) ||
