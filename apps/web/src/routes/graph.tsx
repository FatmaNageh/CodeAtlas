							...
							{topView === "tour" ? (
								<div className="space-y-1">
									...
									<div>
										Current:{" "}
										{activeTourStep
											? `${activeTourStep.rank}/${tourSteps.length}`
											: "—"}
									</div>
								</div>
							) : mode === "path" ? (
								<div className="min-w-[220px] space-y-2">
									<select
										className="w-full rounded-[6px] border border-[var(--b1)] bg-[var(--s1)] px-2 py-1.5 font-mono text-[11px]"
										value={pathFromId}
										onChange={(e) => setPathFromId(e.target.value)}
									>
										{pathCandidates.map((node) => (
									...
									<div className="rounded-[6px] bg-[var(--blue-l)] p-2 font-mono text-[11px] text-[var(--blue)]">
										{currentPath.length > 0
											? currentPath
													.map(
														(id) =>
															filteredNodes.find((n) => String(n.id) === id)
																?.displayLabel ?? id,
													)
													.join(" → ")
											: "No path in current filtered graph"}
									</div>
								</div>
							) : mode === "insight" ? (
								<div className="space-y-1">
								...
								<div className="space-y-1">
									<div>
										{loading
											...
											: `Nodes visible: ${filteredNodes.length}`}
									</div>
